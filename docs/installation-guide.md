...
GitHub Release `v0.1.0` から次を取得する。
...
8. 新しい部位 `.blend` を追加する場合は `Add File Candidate` で選び、collection 一覧のチェックボックス、preview JSON の available collection / object、availableObjectDetails、recommended target、sourceLinkedLibraries を確認する。複数 collection を Link したい場合は対象 collection にチェックを入れる。Reference 用画像やライトは明示 object 名でだけ Link する。`Preview Link` 後に `Link Candidate` を実行する。実行結果の `linkedLibraries` / `indirectLinkedLibraries` で nested Link dependency を確認する。現在の `.blend` は自動保存されないため、結果を確認してから手動で保存する。
