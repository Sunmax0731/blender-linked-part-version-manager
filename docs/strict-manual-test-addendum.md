...
Link reload と Viewport 反映は通過済み。GUI registry editing、link integration、日本語 UI 表示の実機確認で失敗した場合は `Issues/` に再現手順、Blender version、registry、`dist/blender-dry-run-report.json` を記録する。
