...
- File-selector collection candidates should be displayed as checkboxes. Link only checked collections when any collection checkbox state exists, and keep object-name explicit Link available when no collection is checked.
- If a source `.blend` contains nested Blender Links, keep Blender's standard indirect Link behavior and report the dependency chain as `sourceLinkedLibraries` / `indirectLinkedLibraries`; do not localize or auto-save those dependencies.
- Non-model Link targets such as reference image objects, lights, and cameras must be reported with object type and linked only when the user explicitly chooses the object name. Floor/helper objects should be excluded with a report reason.
